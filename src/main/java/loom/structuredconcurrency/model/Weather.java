...
public record Weather(String agency, String weather) {
    ...
    public static Weather readWeather() {
        ...
        try (StructuredTaskScope.ShutdownOnSuccess<Weather> scope = new StructuredTaskScope.ShutdownOnSuccess<>()) {
            // Every time you submit a callable to a structured task scope, it creates a virtual thread and uses it to execute this callable.
            // Because it is cheap to create and block virtual threads, there is no need to pool them. Even if a structured task scope looks like an executor service, it works differently
            // An executor service pools platform threads; a structured task scope creates them on demand and lets them die when they are not needed.
